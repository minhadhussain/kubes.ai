...
begin
  if exists (
    select 1
    from pg_type
    where typname = 'ai_artifact_type'
  ) then
    begin
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'appointments_transaction_id_fkey'
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'tasks_transaction_id_fkey'
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'activities_transaction_id_fkey'
...
create trigger trg_organizations_updated_at before update on public.organizations for each row execute procedure public.set_updated_at();

drop trigger if exists trg_user_profiles_updated_at on public.user_profiles;
create trigger trg_user_profiles_updated_at before update on public.user_profiles for each row execute procedure public.set_updated_at();

drop trigger if exists trg_organization_members_updated_at on public.organization_members;
create trigger trg_organization_members_updated_at before update on public.organization_members for each row execute procedure public.set_updated_at();
...
create trigger trg_listings_updated_at before update on public.listings for each row execute procedure public.set_updated_at();
...
create trigger trg_showing_tours_updated_at before update on public.showing_tours for each row execute procedure public.set_updated_at();
...
create trigger trg_showings_updated_at before update on public.showings for each row execute procedure public.set_updated_at();

drop trigger if exists trg_offers_updated_at on public.offers;
create trigger trg_offers_updated_at before update on public.offers for each row execute procedure public.set_updated_at();
...
create trigger trg_transaction_tasks_updated_at before update on public.transaction_tasks for each row execute procedure public.set_updated_at();

drop trigger if exists trg_documents_updated_at on public.documents;
create trigger trg_documents_updated_at before update on public.documents for each row execute procedure public.set_updated_at();
...
create trigger trg_commissions_updated_at before update on public.commissions for each row execute procedure public.set_updated_at();
...
create trigger trg_ai_artifacts_updated_at before update on public.ai_artifacts for each row execute procedure public.set_updated_at();
...
create policy "organization members can view organizations" on public.organizations for select to authenticated using (public.is_organization_member(id));

drop policy if exists "authenticated users can create organizations they own" on public.organizations;
create policy "authenticated users can create organizations they own" on public.organizations for insert to authenticated with check (owner_user_id = auth.uid());

drop policy if exists "organization admins can update organizations" on public.organizations;
create policy "organization admins can update organizations" on public.organizations for update to authenticated using (public.has_org_role(id, array['owner', 'broker_admin', 'team_admin'])) with check (public.has_org_role(id, array['owner', 'broker_admin', 'team_admin']));

drop policy if exists "users can view own profile or colleagues" on public.user_profiles;
create policy "users can view own profile or colleagues" on public.user_profiles for select to authenticated using (id = auth.uid() or public.shares_organization_with_user(id));
...
create policy "members can view organization memberships" on public.organization_members for select to authenticated using (public.is_organization_member(organization_id));

drop policy if exists "owners and admins can manage organization memberships" on public.organization_members;
create policy "owners and admins can manage organization memberships" on public.organization_members for insert to authenticated with check (
  exists (
    select 1
...
create policy "owners and admins can update memberships" on public.organization_members for update to authenticated using (public.has_org_role(organization_id, array['owner', 'broker_admin', 'team_admin'])) with check (public.has_org_role(organization_id, array['owner', 'broker_admin', 'team_admin']));

drop policy if exists "owners and admins can delete memberships" on public.organization_members;
create policy "owners and admins can delete memberships" on public.organization_members for delete to authenticated using (public.has_org_role(organization_id, array['owner', 'broker_admin', 'team_admin']));

drop policy if exists "members can manage contacts in their organization" on public.contacts;
create policy "members can manage contacts in their organization" on public.contacts for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage leads in their organization" on public.leads;
create policy "members can manage leads in their organization" on public.leads for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage clients in their organization" on public.clients;
create policy "members can manage clients in their organization" on public.clients for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage properties in their organization" on public.properties;
create policy "members can manage properties in their organization" on public.properties for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage listings in their organization" on public.listings;
create policy "members can manage listings in their organization" on public.listings for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage appointments in their organization" on public.appointments;
create policy "members can manage appointments in their organization" on public.appointments for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage showing tours in their organization" on public.showing_tours;
create policy "members can manage showing tours in their organization" on public.showing_tours for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage showings in their organization" on public.showings;
create policy "members can manage showings in their organization" on public.showings for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage tasks in their organization" on public.tasks;
create policy "members can manage tasks in their organization" on public.tasks for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can view activities in their organization" on public.activities;
create policy "members can view activities in their organization" on public.activities for select to authenticated using (public.is_organization_member(organization_id));

drop policy if exists "members can create activities in their organization" on public.activities;
create policy "members can create activities in their organization" on public.activities for insert to authenticated with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage offers in their organization" on public.offers;
create policy "members can manage offers in their organization" on public.offers for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage transactions in their organization" on public.transactions;
create policy "members can manage transactions in their organization" on public.transactions for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage transaction participants in their organization" on public.transaction_participants;
create policy "members can manage transaction participants in their organization" on public.transaction_participants for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage transaction tasks in their organization" on public.transaction_tasks;
create policy "members can manage transaction tasks in their organization" on public.transaction_tasks for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage documents in their organization" on public.documents;
create policy "members can manage documents in their organization" on public.documents for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage notes in their organization" on public.notes;
create policy "members can manage notes in their organization" on public.notes for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage tags in their organization" on public.tags;
create policy "members can manage tags in their organization" on public.tags for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage entity tags in their organization" on public.entity_tags;
create policy "members can manage entity tags in their organization" on public.entity_tags for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage commissions in their organization" on public.commissions;
create policy "members can manage commissions in their organization" on public.commissions for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage expenses in their organization" on public.expenses;
create policy "members can manage expenses in their organization" on public.expenses for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "users can view own notifications" on public.notifications;
create policy "users can view own notifications" on public.notifications for select to authenticated using (user_id = auth.uid() and public.is_organization_member(organization_id));

drop policy if exists "users can update own notifications" on public.notifications;
create policy "users can update own notifications" on public.notifications for update to authenticated using (user_id = auth.uid() and public.is_organization_member(organization_id)) with check (user_id = auth.uid() and public.is_organization_member(organization_id));

drop policy if exists "members can insert notifications in their organization" on public.notifications;
create policy "members can insert notifications in their organization" on public.notifications for insert to authenticated with check (public.is_organization_member(organization_id));

drop policy if exists "admins can view audit logs" on public.audit_logs;
create policy "admins can view audit logs" on public.audit_logs for select to authenticated using (public.has_org_role(organization_id, array['owner', 'broker_admin', 'team_admin', 'coordinator']));

drop policy if exists "members can insert audit logs" on public.audit_logs;
create policy "members can insert audit logs" on public.audit_logs for insert to authenticated with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage ai runs in their organization" on public.ai_runs;
create policy "members can manage ai runs in their organization" on public.ai_runs for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));

drop policy if exists "members can manage ai artifacts in their organization" on public.ai_artifacts;
create policy "members can manage ai artifacts in their organization" on public.ai_artifacts for all to authenticated using (public.is_organization_member(organization_id)) with check (public.is_organization_member(organization_id));
...
drop policy if exists "organization members can read storage objects" on storage.objects;
...
drop policy if exists "organization members can upload storage objects" on storage.objects;
...
drop policy if exists "organization members can update storage objects" on storage.objects;
...
drop policy if exists "organization admins can delete storage objects" on storage.objects;
